...
use syn::{parse_quote, Type, Field, Attribute, Result, Error, Meta, NestedMeta, Lit};
...
pub fn impl_storage(ty: Type, field: &mut Field, attr: Attribute) -> Result<()> {
    ...
    let merkle_type: Type = match arg {
        ...
        Meta::List(l) => {
            let n = l.nested.first().ok_or(Error::new(Span::call_site(), "Internal error."))?;
            if let NestedMeta::Meta(m) = n {
                if let Meta::NameValue(v) = m {
                    let name = v.path.get_ident().ok_or(Error::new(Span::call_site(), "Must set key"))?;
    ...
    let final_type = parse_quote!(baseline::bs3::Storage<#type_, #merkle_type, #ty::Store>);
